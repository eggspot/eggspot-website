/* CASELAB® BFCM — collection filters, sort, and the sticky condensed filter bar. Depends on data.js */
...
function applyCollectionFilters() {
  ...
  if (st.price === 'under10') list = list.filter((p) => p.price < 10);
  if (st.price === '10to20') list = list.filter((p) => p.price >= 10 && p.price < 20);
  if (st.price === 'over20') list = list.filter((p) => p.price >= 20);
  if (st.magsafe) list = list.filter((p) => p.magsafe);
  if (st.q) list = list.filter((p) => (p.name + ' ' + p.desc).toLowerCase().includes(st.q.toLowerCase()));
  /* "Biggest saving" is the sort a Black Friday shopper actually reaches for. */
  if (st.sort === 'save-desc') list.sort((a, b) => savePct(b) - savePct(a));
...
function initCollection() {
  ...
  /* Deep links from the homepage CTAs land on a sort, not just a filter. */
  if (params.get('sort')) {
    collectionState.sort = params.get('sort');
    const sortSel = document.getElementById('sortSelect');
    if (sortSel) sortSel.value = collectionState.sort;
  }
  if (params.get('q')) {
...
function syncFilterBar() {
  ...
  if (mirror) mirror.value = collectionState.sort;
...
function initFilterBar() {
  ...
  mirror.addEventListener('change', () => {
    ...
    if (main) main.value = mirror.value;
    applyCollectionFilters();
  });
