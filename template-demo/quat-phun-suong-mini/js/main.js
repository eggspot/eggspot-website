/* Landing quạt phun sương mini cầm tay — script trang.
...
(function () {
  ...
  var hero = document.querySelector('.qs-hero');
  ...
  if (modal && cards.length) {
    var panel = modal.querySelector('.qs-modal__panel');
  ...
  /* ---------- 7. Đồng bộ combo và gói mua ----------
     Ô combo và radio gói mua là hai nhóm khác nhau (ưu đãi kèm vs số lượng quạt) nên vẫn
     gửi đi cùng lúc được. Riêng ô "Mua 2 quạt" khai báo data-pack vì nó tự nói ra số lượng
  ...
  var comboInputs = $all('.qs-combo__i input');
  var packInputs = $all('.qs-pack input');
  ...
  $all('form[data-order]').forEach(function (form) {
    ...
    form.addEventListener('submit', function (ev) {
      ...
      if (!form.checkValidity()) {
        var bad = flagFields();
        note.className = 'qs-note is-error';
      ...
      note.innerHTML = '<p>Đã nhận đơn: <b>' + (pack ? pack.value : '') + '</b>' +
        ...
        '<a class="qs-note__zalo" href="https://zalo.me/0900123456">Nhắn Zalo 0900 123 456</a>';
      form.reset();
    });
  });
  ...
  if (popup) {
    var popupPanel = popup.querySelector('.qs-popup__panel');
  ...
  var faq = document.querySelector('.qs-faq');
