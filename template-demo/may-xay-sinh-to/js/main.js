/* Landing máy xay sinh tố cầm tay mini — script trang.
   10 phần: GDPR · lọc theo tab · countdown evergreen · gallery ảnh trong đĩa tròn ·
...
(function () {
  ...
  /* ---------- 5. Dải mua nhanh dính đỉnh ----------
     Hiện khi khối hero (đĩa ảnh + thanh giá flash) đã cuộn hết khỏi khung nhìn, để
  ...
  var hero = document.querySelector('.fj-hero');
  ...
  if (modal && cards.length) {
    var panel = modal.querySelector('.fj-modal__panel');
  ...
  /* ---------- 7. Đồng bộ combo và gói mua ----------
     Ô combo (ưu đãi kèm) và radio gói mua (số lượng máy) là hai nhóm khác nhau nên
     vẫn gửi đi cùng lúc được. Ô nào tự nói ra số lượng thì khai báo data-pack — nếu
     để lệch với gói đang chọn thì đơn hàng mâu thuẫn. Hàm này giữ hai bên khớp nhau
     theo cả hai chiều. */
  var comboInputs = $all('.fj-combo__i input');
  var packInputs = $all('.fj-pack input');
  ...
  $all('form[data-order]').forEach(function (form) {
    ...
    form.addEventListener('submit', function (ev) {
      ...
      if (!form.checkValidity()) {
        var bad = flagFields();
        note.className = 'fj-note is-error';
      ...
      note.innerHTML = '<p>Đã nhận đơn: <b>' + (pack ? pack.value : '') + '</b>' +
        ...
        '<a class="fj-note__zalo" href="https://zalo.me/0900123456">Nhắn Zalo 0900 123 456</a>';
      form.reset();
    });
  });
  ...
  if (popup) {
    var popupPanel = popup.querySelector('.fj-popup__panel');
  ...
  var faq = document.querySelector('.fj-faq');
