/* Landing máy rửa xe mini áp lực cao — script trang.
   11 phần: GDPR · lọc theo tab · countdown evergreen · gallery ảnh sản phẩm ·
   dải mua nhanh dính đỉnh · thanh trượt trước/sau · xem nhanh sản phẩm ·
   đồng bộ combo và gói mua · form đặt hàng · popup đặt hàng · FAQ.
...
(function () {
  ...
  /* ---------- 3. Countdown evergreen ----------
     Đồng hồ 9h48m33s tính từ lúc tải trang và tự đặt lại khi về 0 — không có
  ...
  /* ---------- 4. Gallery ảnh sản phẩm ----------
     ...
     và cho phép bấm mũi tên hoặc chấm để nhảy ảnh. Không autoplay. */
  ...
  /* ---------- 5. Dải mua nhanh dính đỉnh ----------
     Hiện khi khối hero đã cuộn hết khỏi khung nhìn, để giá và đồng hồ đếm ngược
     luôn có mặt. Vị trí tính từ getBoundingClientRect nên vẫn đúng khi trang mở
     thẳng vào giữa bài bằng liên kết neo. */
  var stickyBuy = document.querySelector('[data-sticky-buy]');
  var hero = document.querySelector('.aq-hero');
  ...
  /* ---------- 6. Thanh trượt trước / sau ----------
     Điều khiển duy nhất là <input type="range"> phủ trong suốt lên khung ảnh:
     kéo bằng chuột, chạm hay mũi tên bàn phím đều đi qua nó, nên không cần bắt
     pointer thủ công và trạng thái luôn đọc được bằng trình đọc màn hình.
     JS chỉ đổi biến --pos, phần cắt ảnh do clip-path trong CSS lo. */
  var baBox = document.querySelector('[data-ba]');

  if (baBox) {
    var baRange = baBox.querySelector('[data-ba-range]');

    var paintBa = function () {
      baBox.style.setProperty('--pos', baRange.value + '%');
    };

    baRange.addEventListener('input', paintBa);
    paintBa();
  }

  /* ---------- 7. Xem nhanh sản phẩm ----------
  ...
  if (modal && cards.length) {
    var panel = modal.querySelector('.aq-modal__panel');
  ...
  /* ---------- 8. Đồng bộ combo và gói mua ----------
     Ô combo và radio gói mua là hai nhóm khác nhau (quà kèm vs số lượng máy) nhưng
     cả ba ô combo đều tự nói ra số lượng, nên mỗi ô khai báo data-pack. Hàm này giữ
     hai bên khớp nhau theo cả hai chiều để đơn hàng không mâu thuẫn. */
  var comboInputs = $all('.aq-combo__i input');
  var packInputs = $all('.aq-pack input');
  ...
  $all('form[data-order]').forEach(function (form) {
    ...
    form.addEventListener('submit', function (ev) {
      ...
      if (!form.checkValidity()) {
        var bad = flagFields();
        note.className = 'aq-note is-error';
      ...
      note.innerHTML = '<p>Đã nhận đơn: <b>' + (pack ? pack.value : '') + '</b>' +
        ...
        '<a class="aq-note__zalo" href="https://zalo.me/0900123456">Nhắn Zalo 0900 123 456</a>';
      form.reset();
    });
  });

  /* ---------- 10. Popup đặt hàng ----------
     Mở từ nút "Mua ngay" ở hero, thanh hành động và cụm CTA desktop.
  ...
  if (popup) {
    var popupPanel = popup.querySelector('.aq-popup__panel');
  ...
  /* ---------- 11. FAQ: mỗi lần chỉ mở 1 mục ---------- */
  var faq = document.querySelector('.aq-faq');
