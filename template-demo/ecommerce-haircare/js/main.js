/* ============================================
   AURELLE — Premium Hair Care Template
   Cart (localStorage) · variants · filter/sort · motion
   ============================================ */
(function () {
  'use strict';

  var CART_KEY = 'au_cart';
  var FREE_SHIPPING_THRESHOLD = 6000; // cents ($60.00)
  var MAX_QTY = 99;
  ...
  function variantText(line) {
    return [line.option1, line.option2].filter(Boolean).join(' · ') || 'Standard';
  }

  // single shape for a cart line — the card and the product form both feed this.
  // handle + option1 + option2 is the merge key; the Shopify variant id lives on the
  // markup (data-variant-id) and is read at port time, not stored in the demo cart.
  function makeLine(src) {
    return {
      handle: src.handle || '',
      title: src.title || '',
      price: parseInt(src.price, 10),
      option1: src.option1 || '',
      option2: src.option2 || '',
      qty: src.qty || 1,
      img: src.img || ''
  ...
  /* Run `each` once per element, when it first scrolls into view.
     Falls back to running immediately when motion is reduced or IO is missing. */
  function observeAll(elements, each, opts) {
  ...
  function revealElement(el) {
    // no stagger when motion is reduced — the element is already visible via CSS
    var delay = reducedMotion ? 0 : parseInt(el.getAttribute('data-delay'), 10);
    if (delay > 0) {
  ...
  /* Radio-style button group (filter / variant) */
  ...
  function normalizeLine(line) {
    var l = makeLine(line);
    l.qty = clampQty(l.qty);
    l.price = Math.round(Number(l.price));
    return l;
  }
  ...
  function addToCart(line) {
    ...
    var existing = cart.filter(function (l) {
      return l.handle === item.handle && l.option1 === item.option1 && l.option2 === item.option2;
    })[0];
    if (existing) {
  ...
  function initPreloader() {
    ...
    setTimeout(dismiss, 3000); // fallback if load never fires
  }
  ...
  function initHeader() {
    ...
    function setMenu(open) {
      ...
      // the button is the close control while the overlay is up — say so
  ...
  function initCounters() {
    ...
    observeAll(counters, run, { threshold: 0.4 });
  }

  /* ---------------------------------------------
     Product cards: quick add
  --------------------------------------------- */
  function lineFromCard(card) {
    ...
    return makeLine({
      ...
      option1: card.getAttribute('data-option1'),
      option2: card.getAttribute('data-option2'),
      qty: 1,
  ...
  function initProductForm() {
    ...
    var state = {
      option1: form.getAttribute('data-default-option1') || '',
      option2: form.getAttribute('data-default-option2') || ''
    };
    var qtyInput = $('#productQty', form);

    initRadioGroup($('#hairTypeGroup', form), function (value) {
      state.option1 = value;
      var label = $('#selectedHairType');
      if (label) label.textContent = value;
    });

    // size drives the price: each button carries its own data-price in cents
    initRadioGroup($('#sizeGroup', form), function (value, btn) {
      state.option2 = value;
      var label = $('#selectedSize');
      if (label) label.textContent = value;
      var cents = btn.getAttribute('data-price');
      if (cents && priceEl) {
  ...
  function initCartPage() {
    ...
    function buildLine(line, index) {
      ...
      variant.textContent = variantText(line);
  ...
  function init() {
    ...
    initProductForm();
    initCartPage();
    observeAll($$('.reveal'), revealElement, { threshold: 0.12, rootMargin: '0px 0px -40px 0px' });
    updateBadge();
