/* CASELAB® Halloween — conversion psychology layer: cookie bar, exit-intent offer, no-signup wishlist. */
...
const COOKIE_KEY = 'caselab-hw-cookie-ok';
...
const EXIT_KEY = 'podExitIntentShownHw';
const SUBSCRIBED_KEY = 'caselab-hw-subscribed';
...
function buildExitOffer() {
  ...
  el.innerHTML = `
    <div class="exit-offer__card">
      ...
      <span class="mono-label exit-offer__eyebrow">First-order perk</span>
      <h2 id="exitOfferTitle">Leaving without your <em>10%</em>?</h2>
      <p>One email. 10% off your first case. Zero spam, unsubscribe anytime.</p>
      <form class="exit-offer__form" novalidate>
        ...
        <button class="btn btn--pumpkin btn--block" type="submit">Claim My 10% Off</button>
      </form>
      <button class="exit-offer__decline" type="button" data-exit-close>No thanks, I'll pay full price</button>
      <p class="exit-offer__note mono-label">Free US shipping over $25 · 30-day free returns</p>
    </div>`;
...
function initExitOffer() {
  ...
  el.querySelector('form').addEventListener('submit', (e) => {
    ...
    el.querySelector('.exit-offer__card').innerHTML =
      '<p class="form-thanks form-thanks--dark" role="status">Check your inbox — code BOO10 on its way!</p>';
...
const WISHLIST_KEY = 'caselab-hw-wishlist';
...
function showSaveToast(count) {
  ...
  toast.innerHTML = ask
    ? `<p>Want your saved designs on any device?</p>
       <form class="save-toast__form" novalidate>
         ...
         <button class="btn btn--pumpkin" type="submit">Save</button>
       </form>`
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      if (!input.checkValidity()) { input.reportValidity(); return; }
      store.set(SUBSCRIBED_KEY, '1');
      toast.innerHTML = '<p><strong>Done.</strong> Code BOO10 is in your inbox.</p>';
