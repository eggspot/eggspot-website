/* CASELAB® Halloween — header, reveal, parallax, drop urgency, filters, accordion. Depends on data.js */
...
/* The Trick-or-Treat Sale runs out with Halloween night itself, so the deadline is a fixed date
   rather than a rolling weekly drop. Roll to next year once the night is past. */
function nextDropDate() {
  const now = new Date();
  const d = new Date(now.getFullYear(), 9, 31, 23, 59, 59, 0);
...
function initDropUrgency() {
  ...
  if ((target - Date.now()) / 36e5 > URGENCY_WINDOW_H) {
    ...
    if (label) label.textContent = 'Limited spooky run';
    if (claimed) {
      claimed.hidden = false;
      claimed.innerHTML = `Halloween drop — <strong>${dropUnitsClaimed()} of ${DROP_UNITS}</strong> units already claimed`;
    }
    return;
  }
