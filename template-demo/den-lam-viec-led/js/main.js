/* Landing đèn làm việc LED sạc đa năng — script trang.
   ...
   form đặt hàng · popup đặt hàng · FAQ · tab 4 chế độ chiếu sáng.
...
(function () {
  ...
  /* ---------- 2. Tab lọc sản phẩm ----------
     Tab active dùng gạch chân (.is-on), aria-selected phản ánh trạng thái thật.
  ...
  /* ---------- 7. Đồng bộ combo và gói mua ----------
     Ô combo và radio gói mua là hai nhóm khác nhau (ưu đãi kèm vs số lượng đèn) nên vẫn
     gửi đi cùng lúc được. Riêng ô "Mua 2 đèn" khai báo data-pack vì nó tự nói ra số lượng
  ...
  /* ---------- 11. Tab 4 chế độ chiếu sáng ----------
     Tablist thứ hai của trang, tách khỏi nhóm .tk-tab lọc gian hàng. Mỗi nút
     điều khiển đúng một tabpanel qua aria-controls; chỉ tab đang chọn nhận
     tabIndex 0 (roving tabindex) như khuyến nghị của mẫu tabs ARIA. */
  var modeBtns = $all('.tk-mode');

  if (modeBtns.length) {
    var applyMode = function (btn) {
      modeBtns.forEach(function (other) {
        var on = other === btn;
        other.classList.toggle('is-on', on);
        other.setAttribute('aria-selected', on ? 'true' : 'false');
        other.tabIndex = on ? 0 : -1;
    ...
    modeBtns.forEach(function (btn, i) {
      ...
      btn.addEventListener('keydown', function (ev) {
        ...
        if (!step) return;
        ev.preventDefault();
        var next = modeBtns[(i + step + modeBtns.length) % modeBtns.length];
        next.focus();
        applyMode(next);
      });
    });
