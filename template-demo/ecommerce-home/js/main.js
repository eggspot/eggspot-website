/* ============================================
   OAK & EMBER — Home & Living Template
   Cart (localStorage) · variants · filter/sort · motion
   ============================================ */
(function () {
  'use strict';

  var CART_KEY = 'oe_cart';
  ...
  // furniture runs into four figures — group the thousands
  ...
  function normalizeLine(line) {
    return {
      ...
      option1: line.option1 ? String(line.option1) : '',
      option2: line.option2 ? String(line.option2) : '',
  ...
  function initProductForm() {
    ...
    initRadioGroup($('#materialGroup', form), function (value) {
      state.option1 = value;
      var label = $('#selectedMaterial');
      if (label) label.textContent = value;
    });

    initRadioGroup($('#finishGroup', form), function (value) {
      state.option2 = value;
      var label = $('#selectedFinish');
      if (label) label.textContent = value;
    });
    ...
    var addBtn = $('#addToCart', form);
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      addToCart(currentLine(addBtn ? addBtn.getAttribute('data-variant-id') : ''));
    });
