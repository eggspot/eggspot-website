/* CASELAB® Holiday — header, reveal, parallax, drop urgency, filters, accordion. Depends on data.js */
...
/* ---------- holiday urgency (countdown near the cutoff, unit scarcity otherwise) ---------- */
/* The season has exactly one deadline that matters: the last dispatch that still lands under the
   tree. Everything on this page counts down to that, not to a rolling weekly drop. */
const XMAS_CUTOFF = new Date(2026, 11, 18, 18, 0, 0); // Dec 18 2026, 6 PM — last Christmas dispatch

function nextDropDate() {
  return new Date(XMAS_CUTOFF);
}
...
const DROP_UNITS = 500;   // units in the holiday gift run
...
function initDropUrgency() {
  ...
  if ((target - Date.now()) / 36e5 > URGENCY_WINDOW_H) {
    ...
    if (label) label.textContent = 'Christmas cutoff · Dec 18';
    if (claimed) {
      claimed.hidden = false;
      claimed.innerHTML = `Holiday gift run — <strong>${dropUnitsClaimed()} of ${DROP_UNITS}</strong> sets already claimed`;
    }
    return;
  }
