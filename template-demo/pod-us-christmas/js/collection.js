/* CASELAB® Holiday — collection filters, sort, and the sticky condensed filter bar. Depends on data.js */
...
/* Setting `.value` in code never fires `change`, so the custom select wrapping it would keep
   showing the old label. Every programmatic write goes through here. */
...
function applyCollectionFilters() {
  ...
  if (st.price === 'under20') list = list.filter((p) => p.price < 20);
  if (st.price === '20to30') list = list.filter((p) => p.price >= 20 && p.price <= 30);
  if (st.price === 'over30') list = list.filter((p) => p.price > 30);
...
function initCollection() {
  ...
  if (params.get('type')) {
    collectionState.type = params.get('type');
    /* Mirror the deep link into the control, or the toolbar claims "All Products" over 3 results. */
    setSelectValue(document.getElementById('filterType'), collectionState.type);
  }
  if (params.get('q')) {
...
function syncFilterBar() {
  ...
  setSelectValue(document.getElementById('filterBarSort'), collectionState.sort);
...
function initFilterBar() {
  ...
  mirror.addEventListener('change', () => {
    ...
    setSelectValue(document.getElementById('sortSelect'), mirror.value);
    applyCollectionFilters();
  });
