/* CASELAB® Holiday — conversion psychology layer: cookie bar, exit-intent offer, no-signup wishlist. */
...
const COOKIE_KEY = 'caselab-xmas-cookie-ok';
...
const EXIT_KEY = 'podXmasExitIntentShown';
const SUBSCRIBED_KEY = 'caselab-xmas-subscribed';
...
function buildExitOffer() {
  ...
  el.innerHTML = `
    <div class="exit-offer__card">
      ...
      <span class="mono-label exit-offer__eyebrow">Gift-wrap perk</span>
      <h2 id="exitOfferTitle">Leaving before the <em>free gift wrap</em>?</h2>
      <p>One email gets you 10% off plus free gift wrapping on every bundle &mdash; and a reminder before the Dec 18 Christmas cutoff.</p>
      <form class="exit-offer__form" novalidate>
        ...
        <button class="btn btn--lime btn--block" type="submit">Claim 10% + Free Wrap</button>
      </form>
      <button class="exit-offer__decline" type="button" data-exit-close>No thanks, I&rsquo;ll wrap it myself</button>
      <p class="exit-offer__note mono-label">Order by Dec 18 &middot; free US shipping over $35 &middot; 30-day returns</p>
    </div>`;
...
function initExitOffer() {
  ...
  el.querySelector('form').addEventListener('submit', (e) => {
    ...
    el.querySelector('.exit-offer__card').innerHTML =
      '<p class="form-thanks form-thanks--dark" role="status">Check your inbox — code MERRY10 on its way!</p>';
...
const WISHLIST_KEY = 'caselab-xmas-wishlist';
...
function showSaveToast(count) {
  ...
  toast.innerHTML = ask
    ? `<p>Want your saved designs on any device?</p>
       <form class="save-toast__form" novalidate>
         ...
         <input type="email" id="saveToastEmail" placeholder="Enter email → 10% off" required>
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      if (!input.checkValidity()) { input.reportValidity(); return; }
      store.set(SUBSCRIBED_KEY, '1');
      toast.innerHTML = '<p><strong>Done.</strong> Code RADIX10 is in your inbox.</p>';
